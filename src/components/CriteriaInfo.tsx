              ...
              return (
                <AccordionItem key={criteria.code} value={criteria.code} className="border border-slate-200 dark:border-slate-700 rounded-lg px-4 bg-slate-50/50 dark:bg-slate-800/50">
                  <AccordionTrigger className="hover:no-underline py-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-1.5 rounded-md ${(criteria.color || 'bg-slate-400').replace('400', '100')}`}>
                        <Icon className={`w-4 h-4 ${(criteria.color || 'bg-slate-400').replace('bg-', 'text-').replace('400', '600')}`} />
                      </div>
